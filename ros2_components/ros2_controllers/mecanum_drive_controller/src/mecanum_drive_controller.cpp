...
MecanumDriveController::init(const std::string & controller_name)
{
  ...
  try {
    ...
    node->declare_parameter<bool>("linear.y.has_velocity_limits", false);
    node->declare_parameter<bool>("linear.y.has_acceleration_limits", false);
    node->declare_parameter<bool>("linear.y.has_jerk_limits", false);
    node->declare_parameter<double>("linear.y.max_velocity", 0.0);
    node->declare_parameter<double>("linear.y.min_velocity", 0.0);
    node->declare_parameter<double>("linear.y.max_acceleration", 0.0);
    node->declare_parameter<double>("linear.y.min_acceleration", 0.0);
    node->declare_parameter<double>("linear.y.max_jerk", 0.0);
    node->declare_parameter<double>("linear.y.min_jerk", 0.0);
...
controller_interface::return_type MecanumDriveController::update()
{
  ...
  double & linearx_command = received_velocity_msg_ptr_->twist.linear.x;
  double & lineary_command = received_velocity_msg_ptr_->twist.linear.y;
  ...
  if (odom_params_.open_loop) {
    odometry_.updateOpenLoop(linearx_command, angular_command, current_time);
  } else {
    double wheel_fl_pos  = 0.0;
    double wheel_fr_pos  = 0.0;
    double wheel_rl_pos = 0.0;
    double wheel_rr_pos  = 0.0;

    for (size_t index = 0; index < wheels.wheels_per_side; ++index) {
      switch(index)
      {
        case 0:{
          wheel_fl_pos = registered_left_wheel_handles_[index].position.get().get_value();
          wheel_fr_pos = registered_right_wheel_handles_[index].position.get().get_value();
          break;
        }
        case 1:
        {
          wheel_rl_pos = registered_left_wheel_handles_[index].position.get().get_value();
          wheel_rr_pos = registered_right_wheel_handles_[index].position.get().get_value();
          break;
        }
      }
    }
    ...
    odometry_.update(wheel_fl_pos, wheel_fr_pos, wheel_rl_pos, wheel_rr_pos, current_time);
  }
  ...
  rclcpp::Duration dt = current_time - received_velocity_msg_ptr_->header.stamp;

  // Brake if cmd_vel has timeout
  /*if (dt > cmd_vel_timeout_) {
    RCLCPP_WARN_STREAM(logger,"BRAKING!! " << dt.seconds() << " "  << current_time.seconds() << " " << received_velocity_msg_ptr_->header.stamp.sec << " " << cmd_vel_timeout_.count());
    linearx_command = 0.0;
    lineary_command = 0.0;
    angular_command = 0.0;
  }*/
  ...
  limiter_linearx_.limit(
    linearx_command, last_command.linear.x, second_to_last_command.linear.x,
    update_dt.seconds());
  limiter_lineary_.limit(
    lineary_command, last_command.linear.y, second_to_last_command.linear.y,
    update_dt.seconds());
  ...
  //RCLCPP_INFO_STREAM(logger,"VEL COMMAND X " << linearx_command);

  //    Publish limited velocity
  if (publish_limited_velocity_ && realtime_limited_velocity_publisher_->trylock()) {
    auto & limited_velocity_command = realtime_limited_velocity_publisher_->msg_;
    limited_velocity_command.header.stamp = current_time;
    limited_velocity_command.twist.linear.x = linearx_command;
    limited_velocity_command.twist.linear.y = lineary_command;
  ...
  // Compute wheels velocities, asuming wheel separation as lx + ly
  const double velocity_front_left = (1/left_wheel_radius)*(linearx_command - lineary_command - (wheel_separation)*angular_command);
  const double velocity_front_right = (1/right_wheel_radius)*(linearx_command + lineary_command + (wheel_separation)*angular_command);
  const double velocity_rear_left = (1/left_wheel_radius)*(linearx_command + lineary_command - (wheel_separation)*angular_command);
  const double velocity_rear_right = (1/right_wheel_radius)*(linearx_command - lineary_command + (wheel_separation)*angular_command);
  ...
  for (size_t index = 0; index < wheels.wheels_per_side; ++index) {
    ...
    {
    case 0:{
      registered_left_wheel_handles_[index].velocity.get().set_value(velocity_front_left);
      registered_right_wheel_handles_[index].velocity.get().set_value(velocity_front_right);
      break;
    }
    case 1:
    {
      registered_left_wheel_handles_[index].velocity.get().set_value(velocity_rear_left);
      registered_right_wheel_handles_[index].velocity.get().set_value(velocity_rear_right);
      break;
    }
   }
  }
...
CallbackReturn MecanumDriveController::configure_side(
